...
- Pull requests, reviews, and conflicts, because next week your group merges slides into the class repo

::: {.notes}
- Two hours of doing, one hour of Annie.
...
- Every number, graph, and table in the PDF comes from code that ran on the data
- Change the data, render again, everything updates
- A reader with the repo can check any figure
- Pasted-in figures cannot be checked and go stale

## The handoff between languages

```
aggregate_shelter.py  --polars-->  data/shelter_daily.parquet  --arrow-->  02-demo.qmd (R)
```

- Python does the data work: download, clean, aggregate, write Parquet
- R does the graphs and tables: `arrow::read_parquet()`, ggplot2, tinytable
- Parquet keeps the types, so a date is still a date on the other side

## ggplot2 in one slide
...
- Everything after the graphs lecture is about the choices inside those four lines

## tinytable in one slide
...
- A data frame in, a table out, in PDF, HTML, and Word alike
...
- `output-dir: docs` is the line everyone forgets
